...
@section('page-header-preamble')
    {{ $feParams->pagetitle }} / Create Switch via SNMP
@endsection

@section('page-header-postamble')
    @include($data['view']['editHeaderPreamble'])
@endsection
            ...
            <div class="mt-4 card">
                <div class="card-body">
                    <?= Former::open()->method( 'POST' )
                        ->id( 'form' )
                        ->action( route( $feParams->route_prefix.'@store-by-snmp' ) )
                    ...
                    <?= Former::actions(
                            ...
                            Former::secondary_link( 'Cancel' )->href( route( $feParams->route_prefix.'@list') )->class( "mb-2 mb-md-0" ),
                            Former::success_button( 'Help' )->id( 'help-btn' )->class( "mb-2 mb-md-0" ),
                            Former::secondary_link( "Manual / Non-SNMP Add" )->href( route( $feParams->route_prefix.'@create' ) . '?manual=1' )->class( "mb-2 mb-md-0" )
                        );
                    ?>
